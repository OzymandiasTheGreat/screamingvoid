...
export const Messages: React.FC<{
		...
		() =>
			navigation.addListener("focus", () => {
				...
				for (let convo of convos) {
					const last = localStorage.getItem(convo.id);
					if (!last) {
						status.push(true);
					} else {
						status.push(
							(convo.last?.id.localeCompare(last) || 0) > 0
						);
					}
				}
				setUnread(status);
				...
				if (index > -1) {
					status[index] =
						data.message.id.localeCompare(
							localStorage.getItem(data.conversation) || ""
						) > 0;
					list[index].last = {
						id: data.message.id,
	...
	const render = ({
		...
		return (
			<View onContextMenu={onLongPress}>
				<List.Item
					key={item.id}
					onPress={() =>
					...
					descriptionStyle={{
						fontWeight: unread[index] ? "700" : "400",
					}}
